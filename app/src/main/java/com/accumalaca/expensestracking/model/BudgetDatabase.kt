...
@Database(entities = arrayOf(Budget::class, Expense::class), version = 3)   //ngasi tau room tabel DB dgn versi 1
//ganti 3 karena karena ada perubahan sturucture di expense
...
abstract class BudgetDatabase:RoomDatabase(){  //kan kita bikin class BudgetDatabase untuk room ya, otomatis hrs mewarisi RoomDatabase()
    ...
    abstract fun expenseDao(): ExpenseDao //sama kayak diatas ygy
    ...
    companion object{ //static block ala kotlin
        ...
        fun buildDatabase(context:Context) =
            Room.databaseBuilder( //bikin instance DB
                ...
                .fallbackToDestructiveMigration() //apus & buat ulg db saat ganti ver tanpa migrate manual, kau sih si pke ver 1
                .build() //nama sqlite di storage app


        operator fun invoke(context: Context): BudgetDatabase {
            if (instance == null) {
                synchronized(LOCK) {
        ...
        // ga masang return instante tadi, yang bener dong kau
    }
